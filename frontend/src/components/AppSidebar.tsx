...
import { Briefcase, FileText, Home, LogOut, Target } from 'lucide-react'
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
  ...
  return (
    <Sidebar collapsible="icon">
      ...
      <SidebarFooter className="px-3 py-2.5">
        {user && (
          <div className="mb-1.5 flex items-center gap-2.5 rounded-md px-2 py-2 group-data-[collapsible=icon]:hidden">
            <div className="grid size-7 shrink-0 place-items-center rounded-full bg-primary/10 text-xs font-semibold uppercase text-primary">
              {(user.name || user.email).slice(0, 2)}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-medium">{user.name || user.email.split('@')[0]}</p>
        ...
        <a
          href="https://github.com/imadnan4/Hirify-Resume-Matching-"
          target="_blank"
          rel="noopener noreferrer"
          aria-label="View source on GitHub"
          className="group flex items-center gap-2.5 rounded-md px-2 py-2 text-xs text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-sidebar-accent transition-colors"
        >
          <svg
